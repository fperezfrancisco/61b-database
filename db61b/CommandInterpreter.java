...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            ...
            _input.next(")");
            colTitles = new String[V.size()];
            colTitles = V.toArray(new String[0]);
    ...
    Table selectClause() {
        ...
        return table1.select(table2, columns, conditionClause(table1, table2));
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        while (_input.nextIf("where") || _input.nextIf("and")) {
    ...
    Condition condition(Table... tables) {
        String name1 = columnName();
        String rel = _input.next(Tokenizer.RELATION);
        String name2;
        Column col1 = new Column(name1, tables);
        Condition result;
        if (_input.nextIs(Tokenizer.LITERAL)) {
            name2 = literal();
            result = new Condition(col1, rel, name2);
        } else {
            name2 = columnName();
            Column col2 = new Column(name2, tables);
            result = new Condition(col1, rel, col2);
        }
        return result;
    }
